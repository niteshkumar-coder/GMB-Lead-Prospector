...
const SearchForm: React.FC<SearchFormProps> = ({ onSearch, isLoading, progress, status, isGeoActive }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!keyword || (!location && !isGeoActive)) return;
  ...
  return (
    <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm mb-8 relative overflow-hidden">
      {isGeoActive && !isLoading && (
        <div className="absolute top-0 right-0 bg-indigo-600 text-white text-[10px] px-3 py-1 font-black uppercase tracking-widest rounded-bl-lg flex items-center gap-1.5 shadow-sm z-10">
          <span className="relative flex h-2 w-2">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-white opacity-75"></span>
            <span className="relative inline-flex rounded-full h-2 w-2 bg-white"></span>
          </span>
          GPS: Real-Time Distances
        </div>
      )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder={isGeoActive ? "Location (or leave empty for GPS)" : "City Name..."}
              className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all outline-none"
              required={!isGeoActive}
              disabled={isLoading}
            />
          ...
          <div className="flex flex-col">
            <div className="flex justify-between items-center mb-1">
              ...
              <span className="text-xs font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded border border-indigo-100">{radius} km</span>
            </div>
            <div className="mt-2 flex items-center gap-2">
              <span className="text-[10px] text-slate-400 font-bold">1km</span>
              <input
                type="range"
                min="1"
                max="100"
                ...
                disabled={isLoading}
              />
              <span className="text-[10px] text-slate-400 font-bold">100km</span>
            </div>
          </div>
            ...
            <div className="flex justify-between items-end">
              <div className="flex flex-col">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                  {isGeoActive ? 'GPS Position Locked' : 'City Scan Mode'}
                </span>
